  ...
  if (!dbInitialized) {
    try {
      const schemaSql = `
        ...
        -- Safe Alter for daily_records body_fat and circumferences
        ALTER TABLE daily_records ADD COLUMN IF NOT EXISTS body_fat NUMERIC(5, 2);
        ALTER TABLE daily_records ADD COLUMN IF NOT EXISTS neck NUMERIC(5, 2);
        ALTER TABLE daily_records ADD COLUMN IF NOT EXISTS waist NUMERIC(5, 2);
        ALTER TABLE daily_records ADD COLUMN IF NOT EXISTS hip NUMERIC(5, 2);

        -- 15. Create Database Indexes for Performance Optimization
        ...
        CREATE INDEX IF NOT EXISTS idx_progress_images_created_at ON progress_images (created_at);
        ...
        CREATE INDEX IF NOT EXISTS idx_logged_meals_logged_date ON logged_meals (logged_date);
        CREATE INDEX IF NOT EXISTS idx_logged_meals_user_date ON logged_meals (user_id, logged_date);
        
        CREATE INDEX IF NOT EXISTS idx_daily_targets_user_id ON daily_targets (user_id);
        CREATE INDEX IF NOT EXISTS idx_daily_targets_logged_date ON daily_targets (logged_date);
